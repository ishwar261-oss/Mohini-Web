...
/* ══════════════════════════════════════════
   VISUALIZE & EXPLAIN DEMO INTERACTION
   ══════════════════════════════════════════ */
document.addEventListener("DOMContentLoaded", () => {
    const vizInput = document.getElementById("vizDemoInput");
    const vizBtn = document.getElementById("vizDemoBtn");
    const vizResult = document.getElementById("vizDemoResult");
    const vizText = document.getElementById("vizDemoText");
    const vizDiagram = document.getElementById("vizDemoDiagram");

    if (vizBtn) {
        vizBtn.addEventListener("click", async () => {
            const val = vizInput.value.trim().toLowerCase();
            if (!val) return;
            ...
            vizResult.style.display = "flex";

            if (val.includes("request") || val.includes("web") || val.includes("api")) {
                vizText.innerHTML = "<strong>Web Request Flow:</strong><br/>When a user enters a URL, the browser sends an HTTP request to the server. The server processes this request, fetches necessary data from the database, and returns an HTTP response containing the HTML, CSS, and JS back to the browser.";
                const mermaidCode = `graph TD\n    A[Browser / Client] -->|HTTP Request| B[Web Server]\n    B -->|Query| C[(Database)]\n    C -.->|Data| B\n    B -.->|HTTP Response| A`;
                await renderMermaid(mermaidCode);
            } else if (val.includes("sort") || val.includes("algorithm")) {
                vizText.innerHTML = "<strong>Algorithm Sorting Process:</strong><br/>Sorting algorithms systematically compare and swap elements in an array until they are in the desired order. In basic algorithms like Bubble Sort, the largest elements 'bubble' to the top.";
                const mermaidCode = `graph LR\n    A[Unsorted Array] --> B{Compare Elements}\n    B -->|Swap if needed| C[Next Pair]\n    C --> B\n    C -->|End of Array| D[Sorted Array]`;
                await renderMermaid(mermaidCode);
            } else if (val.includes("tree") || val.includes("graph")) {
                vizText.innerHTML = "<strong>Tree Structure:</strong><br/>A tree data structure consists of nodes connected by edges, starting from a root node. Each node can have child nodes, creating a hierarchical relationship.";
                const mermaidCode = `graph TD\n    A[Root Node] --> B[Child 1]\n    A --> C[Child 2]\n    B --> D[Leaf Node]\n    B --> E[Leaf Node]\n    C --> F[Leaf Node]`;
                await renderMermaid(mermaidCode);
            } else {
                // Fallback explanation
                vizText.innerHTML = `<strong>Concept Explanation for '${val}':</strong><br/>This topic typically involves multiple interconnected steps. MOHINI AI dynamically applies formatting so you can understand it at a glance.<br/><br/><em>Note: If a specific concept visual layout isn't formally supported or is too abstract, MOHINI safely provides a structured textual response paired with a basic workflow graph!</em>`;
                const mermaidCode = `graph TD\n    A[Start: ${val}] --> B(Core Process)\n    B --> C{Decision Flow}\n    C -->|Path A| D[Outcome 1]\n    C -->|Path B| E[Outcome 2]`;
                await renderMermaid(mermaidCode);
            }

            vizBtn.textContent = "Generate Visual";
    ...
    async function renderMermaid(code) {
        if (window.mermaid) {
            vizDiagram.innerHTML = "";
            try {
                const { svg } = await window.mermaid.render('mermaid-svg-' + Date.now(), code);
                vizDiagram.innerHTML = svg;
            } catch (err) {
                vizDiagram.innerHTML = "<p style='color: var(--accent-red)'>Failed to render diagram.</p>";
            }
        }
    }
});
